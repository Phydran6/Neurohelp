...
class _AppointmentStartPageState extends State<AppointmentStartPage> {
  ...
  Future<void> _load() async {
    final due = await AppScope.of(context).appointments.pendingNotifications();
    if (!mounted) return;
    setState(() {
      _due = due;
      _loading = false;
                      ...
                      Expanded(
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            final entry = _due[index];
                            return _PhaseCard(
                              appointment: entry.appointment,
                              phase: entry.phase,
                              onAcknowledge: () =>
                                  _acknowledge(entry.appointment, entry.phase),
